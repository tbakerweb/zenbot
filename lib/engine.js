...
module.exports = function (s, conf) {
  ...
  if (so.strategy) {
    ...
    if (s.strategy.onEngineInit) {
      s.strategy.onEngineInit(s);
    }
  }
  ...
  function executeSignal(signal, _cb, size, is_reorder, is_taker, reverseCalled) {
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        // Allow a Strategy to Size the Buy
        if (s.strategy.getBuySize) {
          s.strategy.getBuySize(size, price, s, function (strategySize) {
            if (strategySize) {
              size = n(strategySize).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
            } else {
              size = null
            }
          })
        }
        // Allow Strategy to cancel a Buy based on returning a null size
        if (!size) {
          return cb(null, null)
        }
        ...
        // Allow a Strategy to Size the Buy
        if (s.strategy.getSellSize) {
          s.strategy.getSellSize(size, price, s, function (strategySize) {
            if (strategySize) {
              size = n(strategySize).format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
            } else {
              size = null
        ...
        // Allow Strategy to cancel a Sell based on returning a null size
        if (!size) {
          return cb(null, null)
        }
        if (isOrderTooSmall(s.product, size, price))
  ...
  function writeReport(is_progress, blink_off) {
    let sessionLogId = crypto.randomBytes(4).toString('hex')
    let sessionLog = {
      _id: sessionLogId,
      id: sessionLogId,
      dt: now(),
    ...
    if (s.buy_order) {
      ...
      sessionLog.last_trade_worth = s.last_trade_worth
    ...
    sessionLog.orig_capital = orig_capital
    let orig_price = s.orig_price || s.start_price
    sessionLog.orig_price = orig_price
    if (orig_capital) {
      sessionLog.balance_asset = s.balance.asset
      sessionLog.balance_currency = s.balance.currency
      sessionLog.balance_deposit = s.balance.deposit
      ...
      sessionLog.consolidated = consolidated._value
      let profit = n(consolidated).divide(orig_capital).subtract(1).value()
      sessionLog.profit = profit
      ...
      sessionLog.buy_hold = buy_hold._value
      let over_buy_hold_pct = n(consolidated).divide(buy_hold).subtract(1).value()
      sessionLog.over_buy_hold_pct = over_buy_hold_pct
