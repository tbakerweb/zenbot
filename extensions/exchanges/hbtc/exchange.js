...
module.exports = function hbtc (conf) {
  var public_client, authed_client

  function publicClient () {
    if (!public_client) public_client = new ccxt.hbtc({ 'apiKey': '', 'secret': '', 'options': { 'adjustForTimeDifference': true } })
    return public_client
  }

  function authedClient () {
    if (!authed_client) {
      if (!conf.hbtc || !conf.hbtc.key || conf.hbtc.key === 'YOUR-API-KEY') {
        throw new Error('please configure your hbtc credentials in ' + path.resolve(__dirname, 'conf.js'))
      }
      authed_client = new ccxt.hbtc({ 'apiKey': conf.hbtc.key, 'secret': conf.hbtc.secret, 'options': { 'adjustForTimeDifference': true }, enableRateLimit: true })
    }
  ...
  */
  ...
  function retry (method, args, err) {
    if (method !== 'getTrades') {
      console.error(('\nhbtc API is down! unable to call ' + method + ', retrying in 20s').red)
  ...
  var exchange = {
    ...
    cancelOrder: function (opts, cb) {
      ...
      client.cancelOrder(opts.order_id, joinProduct(opts.product_id)).then(function (body) {
        ...
        // "hbtc {"code":-2011,"msg":"UNKNOWN_ORDER"}"
